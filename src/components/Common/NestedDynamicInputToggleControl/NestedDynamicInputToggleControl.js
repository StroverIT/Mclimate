...
import '../NestedDynamicInput/nestedDynamicInput.scss'

const NestedDynamicInputToggleControl = ({ jsonData, expandable = true, activeKeys = [], defaultActiveKey = '', onSelectItem = ()=>{} }) => {
  useLayoutEffect(() => {
  ...
  const renderAccordionItem = (element, index) => {
    const renderAccordionBody = () => {
      if (Array.isArray(element.content)) {
        return <Accordion activeKey={activeKeys.length == 0 ? [index] : activeKeys} onSelect={onSelectItem} alwaysOpen className={`inner-accordion`}>
  ...
  const renderContent = () => {
    return <Accordion  activeKey={activeKeys} alwaysOpen className={''} id={'wrapper-accordion'}>
...
export default NestedDynamicInputToggleControl